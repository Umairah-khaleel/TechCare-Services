...
public class LaptopRepairActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_laptop_repair);
        ...
        spinnerServiceMethod.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if (position == 1) {
                    ...
                    editTextAddress.setHint("Enter your Home Address for Pickup");
                } else {
        ...
        ivMenu.setOnClickListener(v -> {
            PopupMenu popup = new PopupMenu(LaptopRepairActivity.this, ivMenu);
    ...
    private void submitBooking() {
        ...
        if (deviceModel.isEmpty()) {
            editTextDeviceModel.setError("Enter device model");
            return;
        }
        if (problem.length() < 10) {
            editTextProblem.setError("Problem description must be at least 10 characters");
            return;
        }
        if (date.isEmpty()) {
            editTextDate.setError("Select a valid date");
            return;
        }
        if (timeSlot.equals("Select Time Slot")) {
            Toast.makeText(this, "Please select a time slot", Toast.LENGTH_SHORT).show();
            return;
        }
        if (serviceMethod.equals("Select service method")) {
        ...
        if (serviceMethod.equals("Home Pickup") && address.isEmpty()) {
            editTextAddress.setError("Address is required for Home Pickup");
            return;
        }
        ...
        booking.put("serviceType", "Laptop Repair");
        booking.put("serviceMethod", serviceMethod);

        // (address stored as null)
