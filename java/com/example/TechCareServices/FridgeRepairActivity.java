...
public class FridgeRepairActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_fridge_repair);
        ...
        ivMenu.setOnClickListener(v -> {
            PopupMenu popup = new PopupMenu(FridgeRepairActivity.this, ivMenu);
    ...
    private void submitBooking() {
        ...
        if (deviceModel.isEmpty()) {
            editTextDeviceModel.setError("Enter Fridge model/brand");
            return;
        }
        if (problem.length() < 10) {
            editTextProblem.setError("Describe the issue (min 10 chars)");
            return;
        }
        if (date.isEmpty()) {
            editTextDate.setError("Select a date");
            return;
        }
        ...
        if (serviceMethod.equals("Select service method")) {
            Toast.makeText(this, "Select a service method", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        booking.put("serviceType", "Fridge Repair");
